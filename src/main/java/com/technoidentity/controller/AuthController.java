...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody SignIn signIn) {
        ...
        UserPrincipal userDetails = (UserPrincipal) authentication.getPrincipal();
         UserDto userDto = new UserDto(userDetails.getId(),userDetails.getRole().getId(),userDetails.getRole().getName(),userDetails.getFirstName(),userDetails.getMiddleName(),userDetails.getLastName(),userDetails.getEmail(),userDetails.getProfileImage(), userDetails.getGender(),userDetails.getPrimaryMobile(),"");
    ...
    public ResponseEntity<?> addUser(@Valid @RequestBody SignUp signUp) {
        ...
        return new ResponseEntity(new CommonResponse(user.getId(), sm.format(new Date()), HttpServletResponse.SC_OK, "", "user added successfully", "/api/auth"), new HttpHeaders(), HttpStatus.OK);
